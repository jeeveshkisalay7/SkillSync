...
const JobListings = () => {
    ...
    const fetchJobs = async () => {
        ...
        try {
            ...
            if (locationFilter) queryStr += `&location=${locationFilter}`;
            if (jobTypeFilter) queryStr += `&job_type=${jobTypeFilter}`;
            const res = await api.get(`/external-jobs${queryStr}`);
            setJobs(res.data);
        } catch(e) { 
            console.error(e); 
        } finally {
    ...
    return (
        <div>
            ...
            {/* Filter Bar */}
            <div className="glass-panel" style={{ marginBottom: '32px', display: 'flex', gap: '16px', padding: '16px', flexWrap: 'wrap' }}>
                <div style={{ flex: 2, position: 'relative', minWidth: '250px' }}>
                    ...
                    <input 
                        ...
                        style={{ paddingLeft: '44px', border: 'none', background: '#f1f5f9', width: '100%' }}
                        value={searchTerm}
                ...
                <div style={{ flex: 1, minWidth: '150px' }}>
                    <input 
                        ...
                        className="form-input" 
                        style={{ border: 'none', background: '#f1f5f9', width: '100%' }}
                ...
                <div style={{ flex: 1, minWidth: '150px' }}>
                    <select className="form-input" style={{ border: 'none', background: '#f1f5f9', width: '100%' }} value={jobTypeFilter} onChange={(e) => setJobTypeFilter(e.target.value)}>
                ...
                {jobs.map(job => (
                    <div key={job._id} className="glass-panel" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            ...
                            {job.isExternal ? <span className="tag" style={{background: '#1d4ed8', color: 'white'}}>Source: Live</span> : <span className="tag tag-success">Active</span>}
                        </div>
                        
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                                <Briefcase size={16} /> {job.job_type || 'Full-time'}
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                                <MapPin size={16} /> {job.location || 'Worldwide'}
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
